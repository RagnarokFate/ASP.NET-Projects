...
public class VisaWS : System.Web.Services.WebService {
    ...
    {
        string sqlStr = "Select [VisaNumber], [Cash], [VisaType], [AccountNumber], [Credit], [ExpirationDate] From [BankTbl]";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "asadDataBase.mdb");
        return dt;
    }
    ...
    public bool ValidVisaDetails(int visaNumber, int accNumber, string visaType, string date)
    {
        ...
        string sqlStr = "Select * From [BankTbl] where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType.ToString() + "'and [AccountNumber]='" + accNumber.ToString() + "'";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "asadDataBase.mdb");
        if (dt.Rows.Count>0)
    ...
    {double cash=0;
    int credit = 0;
        string sqlStr = "Select * From [BankTbl] where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType + "'and [AccountNumber]='" + accountNumber.ToString() + "'";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "asadDataBase.mdb");
        if (dt.Rows.Count > 0)
        {
    ...
    {double cash=0;
        ...
        cash = Convert.ToDouble(dt.Rows[0]["Cash"]);
        double result = cash - total;
        string SqlStr = "Update [BankTbl] Set [Cash]='" + result.ToString() + "' Where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType + "'and [AccountNumber]='" + accountNumber.ToString() + "'";
        DBFunctions.ChangeTable(SqlStr, "asadDataBase.mdb");
    }
    ...
    public void UpdateSentTo(string reciever, int quantity, double price)
    {
        ...
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "asadDataBase.accdb");
        if (dt.Rows.Count > 0)
        {
